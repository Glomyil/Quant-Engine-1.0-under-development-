...
**进度**：数据下载 ✅ · 数据层 loader ✅ · 清洗 base ✅ · 执行器 pipeline ✅ · 清洗 `align` ✅ · 清洗 `suspension` ✅ · **清洗 `price` ✅** · `adjust` / `derived` ⬜ · 因子 ⬜ · 回测 ⬜
**最近更新（2026-09-20）**：第三个清洗环节 `price.py` 落地并验收 —— 全库 1,164 万行实测 `is_price_bad = 3` / `is_amt_vol_bad = 60`，与独立体检脚本 **C07=3 / C10=2 / C11=58** 两条路互证；`suspension` 同时补成完整状态机（**40/40**，`--full` **42/42**，全库 `is_suspended.sum() = 282,893` 与体检 C14 一致）。
