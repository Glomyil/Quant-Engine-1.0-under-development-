...
**进度**：数据下载 ✅ · 数据层 loader ✅ · 清洗 base ✅ · 执行器 pipeline ✅ · 清洗 `align` ✅ · 清洗 `suspension` ✅ · 清洗 `price` ✅ · `adjust` / `derived` ⬜ · **因子 + 回测最小闭环 ✅（`sandbox/`，毛水位）**
**最近更新（2026-09-21）**：第一个**最小闭环**跑通 —— `sandbox/` 里 2 个文件（引擎 + 因子清单），在全 A 5,471 只、1,164 万行的数据上跑出 6 个因子的分层结果与多空（毛水位：多空夏普 0.7~1.3、IC 0.02~0.08）。**重点不是数字，而是同时把「离现实多远」列清楚了**（成本 / 融券费 / 中性化 / 多重检验 / 样本外），粗算净夏普会被压到 **0.1~0.4**。
